...
function UpdateSubExamMark({data,onClose}) {
    ...
    const [updateData,setUpdateData] = useState({
        ...
        sub_exam_mark : data ? data.sub_exam_mark : '',
    ...
    useEffect(() => {
        if (data && data.sub_exam_id) {
            setUpdateData(prevUpdateData => ({
                ...prevUpdateData,
                sub_exam_id: data.sub_exam_id
            }));
        }
    }, [data]);
    ...
    if(Object.keys(formErr).length === 0){
        axios.put(`${config.apiurl}/mark/UpdateSubExamMark/${data.sub_exam_mark_id}`,updateData)
        .then((res)=>{
  ...
  return (
    ...
    <Grid container spacing={3} className='mt-3'>
        ...
        {updateData.exam_id === 1 || updateData.exam_id === 2 || updateData.exam_id === 3 ? (
    <>
      <Grid item lg={6} md={6} sm={6} xs={12}>
            <TextField
                fullWidth
        ...
        <Grid item lg={6} md={6} sm={6} xs={12}>
            ...
            value={updateData.sub_exam_mark}
            onChange={handleChangeInput}
            error = {!!errors.sub_exam_mark}
            helperText = {errors.sub_exam_mark}
            />
        </Grid>
        <Grid item lg={6} md={6} sm={6} xs={12}>
            <TextField
...
export default UpdateSubExamMark